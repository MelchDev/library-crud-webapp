...
<?php
	...
	if(isset($_POST["save"]))
	{
		//$sqlCount = $mysql->query("select max(ID_rol + 1) as Conteo from rol");
		$desRol = $_POST["descripcion_rol"];


		call_user_func_array(array('rolController', 'crearRol'), array($desRol));
		...
		//		$vlr = $row['Conteo'];
		//	}
		  // 	$sqlInsert = $mysql->query("INSERT INTO rol VALUES(" . $vlr . ", '" . $desRol ."')");
		//}
	}
?>
	...
	<header>
		<form action="" method="POST">
			<p>Descripcion: 
			<br><br>
		    <input type="text" id="descripcion_rol" name="descripcion_rol" required="required"></p>
		    <br>
		    <p><input type="submit" value="Crear rol" name="save"></p>
		    <br>
		    <p><a href="gestionar_rol.php">Consultar Roles</a></p><br>
		</form>
	</header>
	<footer>
